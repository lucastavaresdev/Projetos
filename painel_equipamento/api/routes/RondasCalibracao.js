...
rondas.get('/ultimos_registros/:tabela/:coluna', (req, res) =>{
    const $query = `SELECT e.nome, DATE_FORMAT(r.${req.params.coluna}, '%d/%m/%Y') as ronda_ultima FROM  ${req.params.tabela} as r inner join equipamentos as e on e.id = r.id_equipamento where e.ronda != 0 or e.calibracao != 0 order by  ${req.params.coluna} desc limit 5;`
    execQuery($query, res);
})
